...
void main(void) {
  ...
  while (1) {
    ...
    // BOTAO 3: liga o watchdog
    if (BTN3 == 0 && btn3Pressionado == 0) {
      btn3Pressionado = 1;
      WDTCONbits.SWDTEN = 1;
      Delay1KTCYx(DELAY_DOS_BTNS);
    } else if (BTN3 == 1 && btn3Pressionado == 1) {
      btn3Pressionado = 0;
    }

    // BOTAO 4: bota pra dormir
    if (BTN4 == 0 && btn4Pressionado == 0) {
      btn4Pressionado = 1;
      WriteCmdXLCD(0x80);  // Comando para colocar na primeira linha
      putrsXLCD("Sleep     ");
      ...
      Sleep();
      Delay1KTCYx(DELAY_DOS_BTNS);
    } else if (BTN4 == 1 && btn4Pressionado == 1) {
      btn4Pressionado = 0;
    }
